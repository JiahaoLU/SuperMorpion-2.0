...
def thread1(clist,thread_over):
    ...
    try:
        td11 = Process(target = thread1, args = (clist,thread_over,),daemon= True)# subthread1 for receiving messenges from client0 and sending it to client1
        td12 = Process(target = thread2, args = (clist,thread_over,),daemon= True)# subthread2 for receiving messenges from client1 and sending it to client0
        td11.start()
        td12.start()
    except:
        print("Error: unable to start thread11 or 12")# if Error, raise it up and quit socket
        TCPSock.close()
        os._exit(0)
